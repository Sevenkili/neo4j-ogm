            ...
            if (writer.type().isArray() || Iterable.class.isAssignableFrom(writer.type())) {
                ...
                if (reader != null) {
                    ...
                    if (paramType.isArray()) {
                        value = FieldWriter.merge(paramType, value, (Object[]) currentValue, elementType);
                    } else {
                        value = FieldWriter.merge(paramType, value, (Collection) currentValue, elementType);
                    }
                }
            }
            writer.write(instance, value);
            ...
            if (writer.type().isArray() || Iterable.class.isAssignableFrom(writer.type())) {
                ...
                if (reader != null) {
                    ...
                    if (writer.type().isArray()) {
                        values = FieldWriter.merge(writer.type(), values, (Object[]) currentValues, valueType);
                    } else {
                        values = FieldWriter.merge(writer.type(), values, (Collection) currentValues, valueType);
                    }
                }
            }
